...
const tags = {
  'el-table': el => {
    ...
    el.__config__.children.forEach(item => {
      // 列属性
      const columnAttrs = Object.keys(item.__config__).map(attr => {
        return `${attr}="${item.__config__[attr]}"`;
      }).join(' ');
      if (item.__config__.children && Array.isArray(item.__config__.children)) {
        const scopedSlots = item.__config__.children.map(item2 => {

          const columnAttrs = Object.keys(item2).map(attr => {
            if (attr == 'round') {
              return `:${attr}="${item2[attr]}"`;
            } else {
              return `${attr}="${item2[attr]}"`;
            }
          }).join(' ');

          const tag = item2.__config__.tag;
          const label = item2.__config__.label;
          return `<${tag} ${columnAttrs}>${label}</${tag}>`;
        });
        str += `\n<el-table-column prop="${item.prop || ''}" label="${item.label}" ${columnAttrs}> 
        ...
        str += `\n<el-table-column prop="${item.prop || ''}" label="${item.label}" ${columnAttrs}> </el-table-column>\n`;
      }
    });
    return `<el-table :data=${confGlobal.formModel + '.' + el.__vModel__} border style="width: 100%">
              ${str}
            </el-table>`;
  },
  'el-button': el => {
